...
func NewMux(handler *Handler) http.Handler {
	...
	authenticated.HandleFunc("GET /panel/v1/dashboard", handler.dashboard)
...
func (handler *Handler) registerAdminRoutes(mux *http.ServeMux) {
	...
	admin.HandleFunc("GET /panel/v1/admin/usage", handler.adminGlobalUsage)
	admin.HandleFunc("GET /panel/v1/admin/usage/records", handler.adminGlobalUsageRecords)
	...
	admin.HandleFunc("POST /panel/v1/admin/invite-codes/{id}/reveal", handler.adminRevealInviteCode)
